...
public class MacDefCalculater implements IMacCalculater{
    ...
    @Override
    public String getMac(int pMacKeyIndex, String pNeedCalMacMsg) {
        byte [] needCalMac = ConvertUtils.hexStringToByte(pNeedCalMacMsg);
        return getMac(pMacKeyIndex,needCalMac);
    }

    @Override
    public String getMac(int pMacKeyIndex, byte[] pNeedCalMacMsgs) {
        String msg = "";
        byte [] macOut = new byte[8];
        getMac(pMacKeyIndex,pNeedCalMacMsgs.length,pNeedCalMacMsgs,macOut,(byte)0x00);
        msg += ConvertUtils.bytesToHexString(macOut);
        return msg;
    }
