package com.urovo.poscommon.utils;
...
//            }
     ...
     */
    public static int Urovo_PCIWriteMKey(KeyType pKeyType, int key_no, int key_len, byte[] key_data) {
        int iRet = 0;
    ...
    public static int writeMasterKeyByKEK(int masterKeyIndex, int kekLen, byte[] kekData, int masterSecLen, byte[] masterSecData) {
        int iRet = Urovo_PCIWriteMKey(KeyType._MASTKEY, masterKeyIndex, kekLen, kekData);
        if(iRet != 0) {
            return -1;
        }
        byte[] masterKeyData = new byte[masterSecLen];
        iRet = Urovo_PciDes(KeyType._MASTKEY, masterKeyIndex,
                masterSecLen, masterSecData,
                masterKeyData, 0x00);
        if(iRet != 0) {
            return -2;
        }
        iRet = Urovo_PCIWriteMKey(KeyType._MASTKEY, masterKeyIndex, masterKeyData.length, masterKeyData);
        if(iRet != 0) {
            return -3;
        }
        return 0;
    ...
    public int GetMac(int macKeyIndex, short DataInLen, byte[] DataIn, byte[] MacOut, byte mode) {
        ...
        if (mode == 0x00) {
            ...
            iRet = Urovo_PciDes(KeyType._MACKEY, macKeyIndex, 8, buf, Macbuf, 1);
            if(iRet != 0) {
                return -1;
            }
     ...
     * @return 返回0成功，非0不成功
     */
    public static int Urovo_PciDes(KeyType pKeyType, int key_no, int inlen, byte[] indata, byte[] desout, int mode) {
        int iRet;
        byte[] reslen = new byte[1];
        byte[] dStartValue = new byte[8];
        int kt = 0;
